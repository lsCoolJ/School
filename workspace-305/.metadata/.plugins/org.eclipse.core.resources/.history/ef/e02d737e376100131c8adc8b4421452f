...
public abstract class AbstractEntity {
    ...
    /** Compares two entities locations and checks if they are going to be sick.
     * @param anE is the abstract entity to be comparing (either a human or a bird).
     * @param aPointList is the list of sick points.
     */
    public void compareToEquals(final AbstractEntity anE, final List<Point> aPointList) {
        if (!anE.isSick() && aPointList.contains(anE.getMyLocation())) {
            final boolean sick = SimParams.GENERATOR.nextBoolean();
            if (sick) {
                anE.setMyIsSick(true);
            }
        }
    }
}
